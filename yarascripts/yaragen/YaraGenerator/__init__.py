...
#Main
def createYara(rulename,filetype,author, description , tags):
  filetypeoptions = ['unknown','exe','pdf','email','office','js-html']
  opt = argparse.ArgumentParser(description="YaraGenerator")
  opt.add_argument("-r", "--RuleName", help="Enter A Rule/Alert Name (No Spaces + Must Start with Letter)")
  ...
  opt.add_argument("-v", "--Verbose",action="store_true", help= "Print Finished Rule To Standard Out")
  opt.add_argument("-f", "--FileType", default='unknown',choices=filetypeoptions, help= "Select Sample Set FileType choices are: "+', '.join(filetypeoptions), metavar="")
  ...
  options.Tags = tags
  options.InputDirectory=pathname+"/test/"
  ...
  with open(options.RuleName + ".yar", 'r') as donerule:  
    yara_rules_file = donerule.read()

  return yara_rules_file
